...
export default function Navbar() {
  const { user, logout, isDemo, activateDemo, isAdmin } = useAuth();
            ...
            <div className="hidden lg:flex items-center gap-1">
              ...
              {isAdmin && (
                <Link
                  ...
                  className="px-3 py-2 rounded-xl text-sm font-medium transition-all"
                  style={{
                    color: location.pathname === '/admin' ? 'var(--text-primary)' : '#8b5cf6',
                    background: location.pathname === '/admin' ? 'rgba(139,92,246,0.12)' : 'transparent',
                    borderBottom: location.pathname === '/admin' ? '2px solid #7c3aed' : '2px solid transparent',
                  }}
                >
                  Admin
                </Link>
