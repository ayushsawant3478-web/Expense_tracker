...
export default function ExpenseForm({ onSuccess }: Props) {
  ...
  const handleSubmit = (e: FormEvent) => {
    ...
    setShowToast(true);
    if (onSuccess) {
      setTimeout(onSuccess, 500); // Small delay before closing modal if in modal
    }
  };
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              <label className="text-xs font-bold uppercase tracking-wider mb-2 block opacity-50 ml-1">Category</label>
              <select
                value={category}
