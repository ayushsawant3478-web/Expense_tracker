...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      console.log("LoginPage after email login - user.role:", user.role);
      if (user.role === 'admin') {
        navigate('/admin');
      } else {
        navigate('/dashboard');
  ...
  const handleGoogleSuccess = async (credentialResponse: any) => {
    try {
      ...
      if (data.token) {
        ...
        console.log("LoginPage after Google login - user.role:", user.role);
        if (user.role === 'admin') {
          navigate('/admin');
        } else {
          navigate('/dashboard');
        ...
        <form className="space-y-5" onSubmit={handleSubmit}>
          ...
          {isLocked && (
            <motion.div
              ...
              animate={{ opacity: 1, y: 0 }}
              className="mb-3 p-4 rounded-xl flex items-start gap-3"
              style={{
            ...
            >
              ...
              <div>
                <p className="text-sm font-bold text-amber-400 mb-0.5">Account Temporarily Locked</p>
